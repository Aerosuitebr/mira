# Importação rápida MRO RJ: findstr nos CSVs da RF + upsert no PostgreSQL.
param([string]$DataRoot = "C:\prospect-portal-data")

$ErrorActionPreference = "Stop"
$rfRoot = Join-Path $DataRoot "rf\extracted"
$tmpLines = Join-Path $env:TEMP "mro-rj-candidates.txt"
$encoding = [System.Text.Encoding]::GetEncoding("ISO-8859-1")
$ufs = @("AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO")

if (Test-Path $tmpLines) { Remove-Item $tmpLines -Force }

Write-Host "Buscando linhas com CNAE MRO nos arquivos RF..."
Get-ChildItem $rfRoot -Directory | Where-Object { $_.Name -like "Estabelecimentos*" } | ForEach-Object {
    $file = Get-ChildItem $_.FullName -File | Where-Object { $_.Name -like "*ESTABELE*" } | Select-Object -First 1
    if (-not $file) { return }
    Write-Host "  findstr em $($file.Name)..."
    cmd /c "findstr /C:`"3316301`" /C:`"3316302`" `"$($file.FullName)`" >> `"$tmpLines`""
}

$rawCount = (Get-Content $tmpLines -ErrorAction SilentlyContinue | Measure-Object -Line).Lines
Write-Host "Candidatas: $rawCount linhas"
...
Get-Content $tmpLines -Encoding Default | ForEach-Object {
    $cols = $_.Split(';', [System.StringSplitOptions]::None)
    if ((Field $cols 5) -ne "02") { return }
    ...
    if (-not $cnaeMain.StartsWith("33163")) { return }
    ...
    if ($cnpj.Length -ne 14 -or $seen.ContainsKey($cnpj)) { return }
    $seen[$cnpj] = $true
    ...
    $rows.Add("SELECT upsert_mro_row($(Sql-Escape $cnpj), $(Sql-Escape $basico), $(Sql-Escape $trade), $(Sql-Escape $cnaeMain), $(Sql-Escape $cnaeSec), $(Sql-Escape $municipio), $(Sql-Escape $uf), $(Sql-Escape $bairro), $(Sql-Escape $street), $(Sql-Escape $cep), $(Sql-Escape $email), $(Sql-Escape $phone), $(Sql-Escape $opened));")
}

Write-Host "Oficinas MRO RJ ativas (CNAE principal): $($rows.Count)"
...
DECLARE
  v_legal text; v_nature text; v_capital numeric; v_porte text;
  v_city text; v_cnae_desc text; v_opened date;
BEGIN
  SELECT legal_name, legal_nature_code, capital_social, company_size_code
    INTO v_legal, v_nature, v_capital, v_porte FROM rf_empresas WHERE cnpj_basico = p_basico;
  ...
  ON CONFLICT (cnpj) DO UPDATE SET
    ...
    cnae_main = EXCLUDED.cnae_main, cnae_secondary = EXCLUDED.cnae_secondary,
    cnae_description = EXCLUDED.cnae_description,
    email = COALESCE(EXCLUDED.email, companies.email),
    phone = COALESCE(EXCLUDED.phone, companies.phone),
    municipality_code = EXCLUDED.municipality_code, city = EXCLUDED.city,
    state = EXCLUDED.state, neighborhood = EXCLUDED.neighborhood,
    street = EXCLUDED.street, zip_code = EXCLUDED.zip_code,
    data_source = 'RECEITA_FEDERAL', updated_at = NOW();
END;
`$`$;
$($rows -join "`n")
...
$tmpSql = Join-Path $env:TEMP "import-mro-rj-fast.sql"
$sql | Set-Content $tmpSql -Encoding UTF8
Get-Content $tmpSql | docker exec -i prospect-portal-postgres psql -U prospect -d prospect_portal -v ON_ERROR_STOP=1

$mro = docker exec prospect-portal-postgres psql -U prospect -d prospect_portal -t -A -c "SELECT COUNT(*) FROM companies WHERE state='RJ' AND cnae_main LIKE '33163%';"
$contacts = docker exec prospect-portal-postgres psql -U prospect -d prospect_portal -t -A -c "SELECT COUNT(*) FROM companies WHERE state='RJ' AND cnae_main LIKE '33163%' AND (COALESCE(email,'') <> '' OR COALESCE(phone,'') <> '');"
Write-Host ""
Write-Host "MRO RJ no banco: $mro (com contato RF: $contacts)"
